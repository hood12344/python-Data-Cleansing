...
def ML_Classification_RandomForestClassifier(train_x, test_x, train_y, test_y):
    ...
    pickle.dump(rf, open("RandomForestClassifier.model", 'wb'))   #儲存權重和模型
    ...
    plt.show()
    return rf
def AI_MLP_Classification_load(test_x,modelFile="MLP_model.json",weightsFile="MLP_model.h5"):
...
def AI_MLP_Classification(train_x, test_x, train_y, test_y):
    ...
    return model
def ML_Classification_load(test_x,modelFile="DecisionTreeClassifier.model"):
    # 決策樹
    model = pickle.load(open(modelFile, 'rb'))
    y_predict = model.predict(test_x)
    print(modelFile,"預測結果:",y_predict)
    return model
...
def ML_Classification_DecisionTreeClassifier(train_x, test_x, train_y, test_y):
    ...
    model=clf
    pickle.dump(clf, open("DecisionTreeClassifier.model", 'wb'))
...
def ML_Classification_KNeighborsClassifier(train_x, test_x, train_y, test_y):
    ...
    pickle.dump(knn, open("KNeighborsClassifier.model", 'wb'))   #儲存權重和模型
...
def ML_Classification_DecisionTreeClassifier_gini(train_x, test_x, train_y, test_y):
    ...
    pickle.dump(clf, open("DecisionTreeClassifier_gini.model", 'wb'))   #儲存權重和模型
...
def ML_Classification_DecisionTreeClassifier_entropy(train_x, test_x, train_y, test_y):
    ...
    pickle.dump(clf, open("DecisionTreeClassifier_entropy.model", 'wb'))
...
def ML_群聚_KMeans(train_x, test_x, train_y, test_y,k=0):
    ...
    pickle.dump(model, open("KMeans.model", 'wb'))
    print("實際", test_y)
